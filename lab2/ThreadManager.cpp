...
void ThreadManager::readInputData() {
    int size = 0;
...
void* ThreadManager::minMaxThread(void* arg) {
    ThreadManager* manager = static_cast<ThreadManager*>(arg);
    manager->findMinMax();
    return NULL;
}

void* ThreadManager::averageThread(void* arg) {
    ThreadManager* manager = static_cast<ThreadManager*>(arg);
    manager->calculateAverage();
    return NULL;
}

void ThreadManager::findMinMax() {
...
void ThreadManager::calculateAverage() {
    if (data_.empty()) return;
...
void ThreadManager::replaceElements() {
    int replace_count = 0;
    for (size_t i = 0; i < data_.size(); ++i) {
...
void ThreadManager::printArray(const std::string& title) const {
    std::cout << title << ": ";
    for (size_t i = 0; i < data_.size(); ++i) {
        std::cout << data_[i] << " ";
    }
