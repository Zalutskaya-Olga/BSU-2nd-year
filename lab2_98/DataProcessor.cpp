...
const int DataProcessor::kMinMaxSleepMs = 7;
const int DataProcessor::kAverageSleepMs = 12;

void DataProcessor::findMinMax(DataManager& dataManager) {
    const std::vector<int>& data = dataManager.getData();
...
void DataProcessor::calculateAverage(DataManager& dataManager) {
    ...
    double average_value = static_cast<double>(sum) / static_cast<double>(data.size());
...
void DataProcessor::replaceElements(DataManager& dataManager) {
    std::vector<int>& data = const_cast<std::vector<int>&>(dataManager.getData());
